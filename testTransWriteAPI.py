...
doc_file = os.path.join(os.path.expanduser("~"), 'Desktop', 'NewWords.txt')
...
def connect(qStr):
    ...
    if not error_flag:
        if not MssageOut == (r['query'] + '\t') and not MssageOut.isspace() and "" != MssageOut:
            print(MssageOut)
            try:
                with open(doc_file, mode='a+', encoding='utf-8') as f:
                    f.write(MssageOut + '\n')
                    f.close()
            except PermissionError:
                print(f'Need higher permission or edit manually.[Path:{doc_file}]')
        else:
...
if __name__ == '__main__':
    ...
    #    queryStr += (" ".join(map(str, args.words[i])))
    # queryStr = input('In put string to search in dictionary.\n')

    if sys.argv[1].startswith('-'):
        if sys.argv[1] == '-f':
            os.system('notepad {}'.format(doc_file))
        elif sys.argv[1] == '-ks':
            APP_KEY = input('Please input a Key.[Can obtain at:https://ai.youdao.com/#/]\n')
            APP_SECRET = input('Please input a Secret.[Can obtain at:https://ai.youdao.com/#/]\n')
            ...
            print("Warning:Other undefined command.")
    else:
        args = sys.argv[1:]
